 ...
 * - is_paid: Filtrar por status de recebimento (opcional, true/false)
 */
 ...
 *   income_date: string (obrigatório, formato: YYYY-MM-DD)
 *   category_id?: string
 *   member_id?: string
 *   source?: string
 *   is_paid?: boolean
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validações básicas
    if (!body.user_id || !body.amount || !body.description || !body.income_date) {
      return NextResponse.json(
        { error: 'Campos obrigatórios: user_id, amount, description, income_date' },
        { status: 400 }
      )
    }
