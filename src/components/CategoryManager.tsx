...
export default function CategoryManager({ userId }: Props) {
  const [categories, setCategories] = useState<ExpenseCategory[]>([])
  const [newCategory, setNewCategory] = useState({ name: '', color: '#007aff' })
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    if (!error) {
      setNewCategory({ name: '', color: '#007aff' })
  ...
  return (
    <div className="glass-card p-6 rounded-3xl animate-slide-up">
      <div className="mb-6">
        <h2 className="text-2xl font-semibold text-apple-gray-700">Categorias de Despesas</h2>
      </div>
